<!--
    View para visualização e edição do volume da caixa.
...
const form = useForm({
    numero: props.volume_caixa.data.numero,
    descricao: props.volume_caixa.data.descricao ?? '',
});

const atualizar = () => {
    form.patch(props.volume_caixa.data.links.update, {
        preserveScroll: true,
        onSuccess: () => {
            flash();
...
const tituloPagina = computed(() =>
    modoEdicao.value === true
        ? __(':attribute: Modo Edição', { attribute: 'Volume da caixa' })
        : __(':attribute: Modo Visualização', { attribute: 'Volume da caixa' })
);
                ...
                <div class="space-y-6">
                    <div class="grid grid-cols-1 gap-x-3 gap-y-6 xl:grid-cols-2">
                        ...
                        <ChaveValor
                            :chave="__('Andar')"
                            :valor="
                                volume_caixa.data.caixa.prateleira.estante.sala.andar.apelido ??
                                volume_caixa.data.caixa.prateleira.estante.sala.andar.numero
                            "
                            icone="layers"
                        />
                    ...
                    <TextAreaInput
                        ...
                        :label="__('Descrição')"
                        :maxlength="255"
                        :placeholder="__('Sobre o volume')"
                        icone="blockquote-left"
                    />

                    <div
                        v-if="volume_caixa.data.links.update"
