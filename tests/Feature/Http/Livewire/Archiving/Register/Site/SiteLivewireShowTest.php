...
test('next and previous are available when viewing individual sites, even when dealing with the first or last record', function () {
    ...
    $first = Site::factory()->create(['name' => 'bar']);
    $second = Site::factory()->create(['name' => 'baz']);
    $last = Site::factory()->create(['name' => 'foo']);

    // has previous and next
    Livewire::test(SiteLivewireShow::class, ['site' => $second])
    ->assertSet('previous', $first->id)
    ->assertSet('next', $last->id);

    // only has next
    Livewire::test(SiteLivewireShow::class, ['site' => $first])
    ->assertSet('previous', null)
    ->assertSet('next', $second->id);

    // has only previous
    Livewire::test(SiteLivewireShow::class, ['site' => $last])
