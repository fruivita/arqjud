...
test('retorna os campos principais e as rotas autorizadas do modelo', function () {
    ...
    $resource = TipoProcessoResource::make($this->tipo_processo);
        ...
        'data' => $this->tipo_processo->only(['id', 'nome', 'descricao'])
            + [
                'links' => [
                    ...
                    'delete' => route('cadastro.tipo-processo.destroy', $this->tipo_processo),
                ],
            ],
    ]);
});
...
test('retorna a quantidade de filhos se houver o eager load da propriedade', function () {
    $resource = TipoProcessoResource::make($this->tipo_processo->loadCount(['caixas']));
...
test('retorna o resource vazio se o modelo for nulo', function () {
    expect(TipoProcessoResource::make(null)->resolve())->toBeEmpty();
});
